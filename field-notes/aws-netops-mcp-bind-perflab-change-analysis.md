...
Ivan at ipSpace has been busy. [netlab 26.09](https://blog.ipspace.net/2026/09/netlab-26-09/) added Syslog support (client and server, with dnsmasq doing the server side), plus generic prefix sets for cleaner ACL/prefix-list writing. He also filed an [OSPF MTU saga](https://blog.ipspace.net/2026/09/ospf-mtu-saga/) — every certification exam ever asks about "OSPF stuck in ExStart," and it turns out the arcane details are worse than any of us remembered. Si has lived exactly this saga on real kit: [Of course it's MTU, but *how* is it MTU?](https://simonpainter.com/of-course-it-is-mtu/) is the war-story companion piece — Juniper SRX slotted into a Nexus 7K perimeter, jumbo frames on VLAN-tagged interfaces, and the outside VRF's adjacency stuck at LOADING while the inside came up clean. The bite is the Cisco L2-vs-L3 MTU accounting: the L3 IP MTU matched on both sides, but on a Nexus VLAN interface you also need the L2 switchport MTU to keep up, and the platform default *doesn't appear in the running config* even though the docs imply the `show run all | i jumbomtu` command will surface it. Read Ivan's piece for the theory, then read Si's for what it looks like at 2am. And Ivan's [continuing the Vagrant/libvirt sunset](https://blog.ipspace.net/2026/09/sunsetting-vagrant-libvirt/) — netlab is drifting toward container-first tooling and the reasons are, as ever, painfully practical.
...
- Si's back-catalogue: [MCP security controls](https://simonpainter.com/mcp-security-controls) · [SD-WAN: A Strategic Step Toward Zero Trust](https://simonpainter.com/blog/sdwan-strategic-step-to-ztna) · [Of course it's MTU, but *how* is it MTU?](https://simonpainter.com/of-course-it-is-mtu/)
