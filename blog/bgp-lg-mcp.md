...
I was having a chat with a long-time friend, [Adam Sharif](https://www.linkedin.com/in/adamsharif/), about AI and in the conversation I mentioned that I had been meaning to write a BGP route server MCP proxy for a while. Cue an A-Team style musical montage and another evening lost to an ADHD hyperfocus session.
<!-- truncate -->
...
The thing is, now that we're all lazy and used to asking an AI everything, I thought what if I could just ask "What's the AS path to 8.8.8.8?" rather than logging in and typing the command "show ip bgp 8.8.8.8" and figuring out what the results mean.
...
The [Route Views Project](https://www.routeviews.org/) at the University of Oregon maintains several public route servers scattered around the world. They're free to use, mostly always available, and updated in real time from live BGP feeds. They're invaluable for:
...
![Screenshot showing BGP route view servers being retrieved](img/bgp-lg-mcp/bgp_route_view_servers_retrieval_screenshot.png)
...
The server is built with [FastMCP](https://github.com/jlowin/fastmcp), which means it supports both STDIO and streaming HTTP transports out of the box. STDIO is perfect for local use with Claude Desktop, while the HTTP transport lets you run the server remotely and connect to it from anywhere. No extra code required—FastMCP handles both transport methods automatically.
...
![Route view server comparison showing results for 3.3.3.0/24](img/bgp-lg-mcp/route_view_server_comparison_3_3_3_summary.png)
...
The server is working and can be deployed. The Route Views Project maintains a stable set of looking glasses that have been running for years, so the infrastructure is reliable. The main expansion would be adding support for more BGP query types: things like filtering by AS path or origin, or examining more detailed route attributes.
