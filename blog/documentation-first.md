...
Documentation first approach gives a clear hierarchy. Returning to the fizzbuzz example above it is easy to see that if the ticket says that 'jazz is not being returned when n is a multiple of 7' then we can immediately tell that this is a new requirement and not breakfix. If the ticket says that 15 is returned when n is 15 then we probably already have a test failure, and if we don't we can add one as part of fixing the problem.
Translating this into infrastructure if the ticket says that 'the network is running slow' with 1000 people on the site for the event, and the definition says the site supports 300 users we can set expectations correctly, find out why someone scheduled an event for 1000 people on a site designed for 300 and also create a hotfix if there is time to increase the capacity of the site. If the definition says the site should support 1000 people then there should be a test in place to ensure that this is the case.
