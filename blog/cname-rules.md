...
So it makes a lot of sense that you wouldn't want to turn a 4 byte IP address into a human readable text string and then back into binary with bytes for every unicode character. Things get even more absurd with an IPv6 address which would require 16 bytes of binary data but would be represented as a much longer string with colons and hexadecimal digits. The MX record's EXCHANGE field is designed to be a domain name, not an IP address, so that it could support IPv4, IPv6 and any other future IP version without needing to change the MX record format.
