...
The .AL DNSSEC rollover story I covered on 19 July already showed how thin the operational margin around DNSSEC has become. Germany makes the same point from a different angle: the signatures are only as good as the process that publishes them. Si's [Hybrid Cloud Reference Architectures](https://simonpainter.com/blog/hybrid-cloud-dns) piece from May is worth a re-read alongside this — the "who signs what, and who checks the signer" question is exactly the sort of thing that ends up being one team's tribal knowledge until it isn't.
...
Si covered the launch context in the [.AL DNSSEC roundup](https://simonpainter.com/blog/al-dnssec-front-door-edge-actions-aws-extranet) two weeks back, and it links to his broader [Where to WAF](https://simonpainter.com/blog/where-to-waf) piece on picking edge vs application layer. This week's post fills in the engineering choices behind the pitch.
...
Si called the direction of travel back in the [post-quantum clock](https://simonpainter.com/blog/quiet-week-cloud-wan-routing-policy-post-quantum-clock) post on 28 June (EO 14412 sets the regulatory deadline at 2030) and again in the [ML-DSA pragmatism](https://simonpainter.com/blog/united-airlines-nat-front-door-tenant-isolation-ml-dsa-pragmatism) piece on 12 July (Cloudflare arguing publicly that ML-DSA is what we've got, ship it). This week is that argument in production. If you run anything behind a CDN or reverse proxy, "post-quantum" is no longer something happening in a lab — it's the checkbox on the origin pull config screen.
