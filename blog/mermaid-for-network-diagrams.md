...
I've drawn a lot of network diagrams over the years. Visio, Lucidchart, draw.io, the back of a napkin and, on one memorable occasion, a whiteboard that turned out to be a wall. These days I draw almost all of them in Mermaid, and I've come to think it's the right tool for most of what network engineers need to say with a picture.
<!-- truncate -->
...
The second argument is the one that convinced me in the first place. Mermaid is text. It lives in a `.md` file or a `.mmd` file alongside the rest of the documentation, which means it lives in git alongside the rest of the code.
...
It also means the diagram can go through the same pipeline as everything else. I've written before about [accessibility checks in CI](/accessibility-and-ci) and the same idea applies here. If your diagram is code, you can lint it, test that it renders, and fail the build if someone breaks it. My blog does exactly that.
...
The third argument is the one I've come round to most slowly, and I think it's the most important. Ask yourself what a network engineer does. Strip away the vendor names and the acronyms and it comes down to this: we connect things. Physical connections between devices. Logical connections between subnets. Peerings between VNets. Tunnels between sites. Sessions between routers.

Everything we build is a graph. Nodes and edges. Some of the nodes contain other nodes: a rack contains switches, a VNet contains subnets, a region contains availability zones. That's it. That's the whole job when you squint at it.
...
I don't like Mermaid because it makes pretty pictures. It doesn't, really. I like it because it makes me focus on the right things. It stops me fiddling with icons and forces me to describe connections. It puts my diagrams under the same review process as my code. And it models a network the way I already think about one: things inside things, joined by lines.
