...
What flat-rate Direct Connect does is widen the gap between the two. For traffic to Regions covered by the selected tier, the private circuit is now not just cheaper per gigabyte than the internet path, it's a fixed cost with no per-gigabyte component; traffic outside the tier and excluded SiteLink or transit traffic can still incur separate charges. If you're still running production hybrid traffic over a VPN because the Direct Connect business case never quite stacked up, this is the announcement that might change the answer.
