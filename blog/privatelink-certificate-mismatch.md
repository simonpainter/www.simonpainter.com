...
**Narrow conditional forwarding scope.** This is the one your on-prem DNS admins should appreciate. Because the private override happens at the `privatelink.blob.core.windows.net` layer, your corporate DNS only needs to intercept that subdomain. If Azure had instead published private records directly under `blob.core.windows.net`, you'd have to hijack the entire namespace - and break resolution for every other storage account on the planet that you *don't* have a private endpoint for. The [DNS integration docs](https://learn.microsoft.com/en-us/azure/private-link/private-endpoint-dns-integration) carry an explicit caution against overriding zones that are actively resolving public endpoints, and the whole conditional-forwarder pattern for hybrid estates hangs off the privatelink layer being separable. I covered the mechanics of the resolver side of this in [Hybrid Cloud Reference Architectures](https://www.simonpainter.com/hybrid-cloud-dns).
